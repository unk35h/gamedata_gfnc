...
, [proto_object_CounterModule.CounterModuleDailyChallenge] = function(data, moduleRefreshTables)
  -- function num : 0_5 , upvalues : _ENV, NoticeData, JumpManager, SectorStageDetailHelper
  local moduleId = data.specifyId
  ;
...
, [proto_object_CounterModule.CounterModuleWeeklyChallengeFresh] = function(data, moduleRefreshTables)
  -- function num : 0_7 , upvalues : _ENV, NoticeData, JumpManager, SectorStageDetailHelper
  if not moduleRefreshTables[proto_object_CounterModule.CounterModuleWeeklyChallengeFresh] then
    local WCRefreshTable = {isSendMsg = false}
  end
  ;
...
, [proto_object_CounterModule.CounterModuleRefreshableDungeon] = function(data, moduleRefreshTables)
  -- function num : 0_8 , upvalues : _ENV, NoticeData, JumpManager
  ...
  if FunctionUnlockMgr:ValidateUnlock(proto_csmsg_SystemFunctionID.SystemFunctionID_DailyDungeon) and isNew then
    NoticeManager:AddNotice((NoticeData.CreateNoticeData)(data.nextExpiredTm, (NoticeManager.eNoticeType).DailyDungeon, {jumpType = (JumpManager.eJumpTarget).Sector, argList = nil}, nil, nil))
  end
