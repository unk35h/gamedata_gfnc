...
ExplorationResidentStoreCtrl.CheckEpResidentStore = function(self, closeCallback)
  ...
  if curRoomData:GetIsInBigBossRoom() and self.dynPlayer ~= nil then
  ...
  if self:HasEpResidentStore() and (self.formBattle or (self.dynPlayer):GetOperatorDetailState() == proto_object_ExplorationCurGridState.ExplorationCurGridState_MonsterLvUpgrade or isFirstRoom) then
    ...
    -- function num : 0_8_0 , upvalues : isFirstRoom, self, closeCallback
...
ExplorationResidentStoreCtrl.ReqEpResidentStoreLvUp = function(self, callBack)
  ...
  if (self:GetResidentStoreCfg()).maxLevel <= self.storeLevel then
    return 
  end
...
ExplorationResidentStoreCtrl.GetResidentStoreCfg = function(self)
...
ExplorationResidentStoreCtrl.GetResidentStoreRefreshPrice = function(self)
  -- function num : 0_17 , upvalues : _ENV, EpStoreRoomUtil
