...
ExplorationPlayerCtrl.CheckEpRoomAccess = function(self, roomData)
  ...
  if unlimieChoose and currentRoom.x < roomData.x and roomData.x <= mapData.depth then
    return true
  end
...
ExplorationPlayerCtrl.OnEpOperatorDiff = function(self, epOp)
  ...
  if epOp.state == proto_object_ExplorationCurGridState.ExplorationCurGridState_DropAlg and not ExplorationManager:IsInExplorationLight() and not ExplorationManager:IsInGuardExp() then
    (self.epCtrl):DiscardChip()
  end
    ...
    do
      if epOp.state == proto_object_ExplorationCurGridState.ExplorationCurGridStateReplaceChip then
        ...
        if epOp.state == proto_object_ExplorationCurGridState.ExplorationCurGridState_QuestSelect then
          ...
          do
            if epOp.state == proto_object_ExplorationCurGridState.ExplorationCurGridState_OpeningBuffSelect then
              local isInBattleSceen = ((self.epCtrl).sceneCtrl):InBattleScene()
              if not isInBattleSceen then
...
ExplorationPlayerCtrl.OnSpecEffectDiff = function(self, diffData)
...
ExplorationPlayerCtrl.OnWcEpScoreChange = function(self, epWCscore)
  -- function num : 0_20
