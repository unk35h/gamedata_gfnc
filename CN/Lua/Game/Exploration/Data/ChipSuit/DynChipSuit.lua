...
DynChipSuit.ExecuteSuitChipForList = function(self, heroList, dynPlayer)
  ...
  for k,chipBattleData in pairs(self.__suitLevel) do
    ...
    for k,role in pairs(validRoleList) do
      chipBattleData:ExecuteChipBattle(R16_PC17)
    end
  end
end
...
DynChipSuit.__ExecuteSuitChip = function(self, chipBattleData, dynPlayer)
  ...
  if chipBattleData:IsValidDynPlayer() then
    ...
    for k,role in pairs(validRoleList) do
      chipBattleData:ExecuteChipBattle(R11_PC21)
    end
  end
end
...
DynChipSuit.__RollBackSuitChip = function(self, chipBattleData, dynPlayer)
  ...
  if chipBattleData:IsValidDynPlayer() then
    ...
    local validRoleList = chipBattleData:GetValidRoleList(dynPlayer.heroList, eBattleRoleBelong.player, dynPlayer:GetSpecEffectMgr())
    for k,role in pairs(validRoleList) do
      chipBattleData:RollbackChipBattle(R11_PC21)
    end
  end
end
...
DynChipSuit.UpdateChipSuitTotalQty = function(self, dynPlayer)
...
DynChipSuit.GetTotalQty = function(self)
  -- function num : 0_16
