...
UIEventBattlePassPurchase.InitBattlePassPurchaseStaticUI = function(self)
  ...
  (self.resloader):LoadABAssetAsync(respath, function(prefab)
    -- function num : 0_2_0 , upvalues : _ENV, self, BattlePassEnum
    ...
    local className = (BattlePassEnum.ReHolderStyleMapping)[((self.passInfo).passCfg).hero_res]
    if className ~= nil then
      local styleClass = require("Game.BattlePass.UI.ReHolderStyle." .. className)
      local styleNode = (styleClass.New)()
      styleNode:Init(go)
      styleNode:InitReHolderStyle((self.passInfo).passCfg, self.resloader)
    end
  end
)
end
