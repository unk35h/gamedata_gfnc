...
UINAvgHeroPic.InitAvgHeroPicParam = function(self)
  ...
  if imgCfg.ripple then
    local ripple = not imgCfg.comm
  end
...
UINAvgHeroPic.__ShowRipple = function(self, show)
  ...
  if show and self._rippleMat == nil then
    ...
    do
      ...
      if not show and self._rippleMat ~= nil then
        ((self.ui).rawImg).material = self.heroMatDefault
...
UINAvgHeroPic.AvgHeroDissolveTween = function(self, duration)
  -- function num : 0_8 , upvalues : _ENV, CS_Ease
  local avgWindow = UIManager:GetWindow(UIWindowTypeID.Avg)
  if avgWindow == nil then
    return 
  end
...
UINAvgHeroPic.AddAvgImgTween = function(self, tweenCfg)
  -- function num : 0_13 , upvalues : AvgImgTweenUntil, _ENV
...
UINAvgHeroPic.AvgImgTweenDoEnd = function(self)
  -- function num : 0_19
  if self.sequence ~= nil and (self.sequence):IsPlaying() then
    (self.sequence):Kill()
  end
