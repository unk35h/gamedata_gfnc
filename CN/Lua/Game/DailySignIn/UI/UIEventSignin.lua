...
UIEventSignin.OnInit = function(self)
  -- function num : 0_0 , upvalues : _ENV, cs_ResLoader, UINEventSigninRewardItem, UINBaseItemWithReceived, JumpManager
...
UIEventSignin.__InitRewardList = function(self, allWait2PickDayDic)
  ...
  for i = 1, monthDayNum do
    ...
    item:InitSigninRewardItem(R12_PC81, self.__SelectRewardItemCallback)
    if i > pickedDayNum then
      do
        item:SetIsReceived(R12_PC81)
        ...
        if pickedDayNum == i then
          ...
          self:__SelectRewardItem(R12_PC81)
        end
...
UIEventSignin.ShowCollectSignInReward = function(self, mailController, notPickedMailUIDsList, index)
  ...
  if thisRewardItem == nil then
    ...
    -- function num : 0_7_0 , upvalues : _ENV, self, mailUids, mailController, notPickedMailUIDsList, index, thisRewardItem
...
UIEventSignin._CollectSignInreward = function(self, mailUids, mailController, notPickedSinginMailUIDList, index)
  ...
  self:__GetAllMailRewars(mailUids, 1, function()
    -- function num : 0_8_0 , upvalues : _ENV, index, notPickedSinginMailUIDList, self, mailController, mailUids, CommonRewardData
    UIManager:ShowWindowAsync(UIWindowTypeID.CommonReward, function(window)
      -- function num : 0_8_0_0 , upvalues : index, notPickedSinginMailUIDList, self, mailController, _ENV, mailUids, CommonRewardData
      local rewardIds = {}
      local rewardNums = {}
      ...
      if leftday > 0 and leftday <= (ConfigData.game_config).monthCardTipsDay then
        CRData:SetCRMonthCardTimeTips()
      end
...
UIEventSignin.ShowMonthCardLeftNum = function(self)
  ...
  if leftday > 0 then
    ...
    if leftday <= (ConfigData.game_config).monthCardTipsDay then
      needTips = true
    end
  else
    ;
...
UIEventSignin.OnClickCloseSign = function(self)
