...
local OpenTaskOrder = {[(TaskEnum.eTaskType).DailyTask] = 1, [(TaskEnum.eTaskType).WeeklyTask] = 2, [(TaskEnum.eTaskType).MainTask] = 3, [(TaskEnum.eTaskType).SideTask] = 4, [(TaskEnum.eTaskType).LargeActivityTask] = -1, [(TaskEnum.eTaskType).HeroActivityTask] = -2}
UITask.OnInit = function(self)
  -- function num : 0_0 , upvalues : _ENV, cs_ResLoader, UINTaskPageTog, UINTaskSubPageTog, UINTaskList, UINTaskPeroidNode, UINTaskParticleNode, UINTaskBattlePass, UINTimeLimitTasklimitNode
...
UITask.RefreshPages = function(self)
  -- function num : 0_1 , upvalues : _ENV, TaskEnum, OpenTaskOrder
    ...
    if isDailyTaskUnlock or not (table.contain)(group, 4) then
      ...
      for index,taskTypeId in ipairs(group) do
        ...
        if OK then
          ...
          if RedDotCount > 0 and OpenTaskOrder[taskTypeId] ~= nil and OpenTaskOrder[autoSelectPage] ~= nil and (not isReddotAutoSelect or OpenTaskOrder[taskTypeId] < OpenTaskOrder[autoSelectPage]) then
      ...
      if typeId == (TaskEnum.eTaskType).MainTask or (TaskEnum.IsActivityTask)(typeId) then
        ...
        do
          ...
          do
            ...
            self:SetTaskNodeParent(typeId ~= (TaskEnum.eTaskType).MainTask and (TaskEnum.IsActivityTask)(typeId))
