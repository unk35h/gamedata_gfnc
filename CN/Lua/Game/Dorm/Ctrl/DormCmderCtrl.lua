...
DormCmderCtrl.CheckCmderInteract = function(self)
  ...
  for _,interPoint in pairs(interPointList) do
    ...
    if interPointData:GetInterAnimType() <= 0 and (cs_UnityUtility.IsCloseDistance)((self.cmderEntity).transform, interPoint:GetPointStartPos(), CheckFntDistance) then
  ...
  if (((self.dormCtrl).houseCtrl).curHouse):GetRoomCount() > 1 then
