...
AllTaskData.ctor = function(self)
  ...
  for k,stc in pairs(ConfigData.task) do
    ...
    if self:IsNormalType(stc.type) and (self.normalTaskDatas)[stc.type] == nil then
      (self.normalTaskDatas)[stc.type] = {}
    end
    -- DECOMPILER ERROR at PC97: Confused about usage of register: R6 in 'UnsetPending'

    if self:IsTimeLimitType(stc.type) and (self.timeLimitTaskDataDic)[stc.type] == nil then
...
AllTaskData.AddTaskData = function(self, taskData)
  ...
  if not (self.taskTypeDatas)[stcData.type] then
    local typeDic = {}
  end
    ...
    if self:IsSectorType(stcData.type) then
      ...
      if self:IsAchivType(stcData.type) then
        ...
        if self:IsNormalType(stcData.type) then
          ...
          if self:IsTimeLimitType(stcData.type) then
...
AllTaskData.UpdateAllTaskData = function(self, forceRefresh, changedTaskTypeDic)
  -- function num : 0_7 , upvalues : _ENV
  self:__ExpireData()
  local isNormalHasChange, isSectorHasChange, isAchivLevelHasChange, isWCHasChange, isTimeLimitHasChange = nil, nil, nil, nil, nil
      ...
      if self:IsNormalType(taskType) then
        ...
        if self:IsAchivType(taskType) then
          ...
          if self:IsWeeklyChallengeType(taskType) then
            ...
            if self:IsTimeLimitType(taskType) then
              isTimeLimitHasChange = true
            else
              if self:IsSectorType(taskType) then
...
AllTaskData.InitNormalTaskRedDot = function(self, changedTaskTypeDic)
...
AllTaskData.InitTimeLimitTaskRedDot = function(self, changedTaskTypeDic)
...
AllTaskData.InitAchivLevelRedDot = function(self, changedTaskTypeDic)
...
AllTaskData.IsSectorType = function(self, type)
  -- function num : 0_16 , upvalues : TaskEnum, _ENV
...
AllTaskData.IsTimeLimitType = function(self, type)
  -- function num : 0_20 , upvalues : TaskEnum
  if not (self._taskTypeShowInMain)[type] then
    return false
  end
  do return ((TaskEnum.eTaskType).TimeLimitTask <= type and type <= self.__timeLimitTypeCeiling) or ((TaskEnum.eTaskType).ActivityTask <= type and type < self.__activityTypeCeilling) end
  -- DECOMPILER ERROR: 1 unprocessed JMP targets
end

AllTaskData.AllTaskOnTaskUpdate = function(self, taskData)
  -- function num : 0_21 , upvalues : _ENV, NoticeData, JumpManager, TaskEnum
...
AllTaskData.AllTaskOnTaskCommit = function(self, stcTask)
  -- function num : 0_22 , upvalues : TaskEnum
...
AllTaskData.GetTaskData4Home = function(self)
  -- function num : 0_28 , upvalues : _ENV, TaskEnum
...
AllTaskData.__GetMainTask4Home = function(self)
  -- function num : 0_29 , upvalues : _ENV, TaskEnum
  local taskData = nil
...
AllTaskData.GetTaskDataById = function(self, taskId, containCompleted)
  -- function num : 0_30 , upvalues : _ENV, TaskData
...
AllTaskData.GetTaskDataDicByType = function(self, taskType)
  -- function num : 0_31 , upvalues : _ENV
  if not (self.taskTypeDatas)[taskType] then
