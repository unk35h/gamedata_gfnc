...
SectorItemEntity.RefreshSctStageProress = function(self)
  ...
  for k,id in ipairs(sectorIdList) do
    ...
    if stageCfg.difficulty ~= eSectorStageDifficult.Endless then
      do
        ...
        if (self.sectorCfg).task_is_open then
  ...
  if (self.sectorCfg).task_is_open then
    (self.uiSctProgressStage):SetSctChallengeProgress(true, curChallengeTaskNum, allChallengeTaskNum)
  else
    ;
    (self.uiSctProgressStage):SetSctChallengeProgress(false)
  end
