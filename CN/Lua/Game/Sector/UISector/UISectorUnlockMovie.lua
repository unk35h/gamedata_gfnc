...
UISectorUnlockMovie.TryPlayVideo = function(self, sectorId)
  ...
  if sectorCtrl.needPlayEndVideo ~= nil then
    ...
    (NetworkManager:GetNetwork(NetworkTypeID.Object)):CS_User_ModifySectorUnlockVideo(unlockSectorId)
    -- DECOMPILER ERROR at PC10: Confused about usage of register: R0 in 'UnsetPending'

    PlayerDataCenter.sectorUnlockVideo = unlockSectorId
    ;
    (((self.ui).img_Video).gameObject):SetActive(true)
    ;
    (self.moviePlayer):PlayVideo(path, function()
      -- function num : 0_1_0_0 , upvalues : self, sectorCtrl, unlockSectorId, sectorId, _ENV
      ((self.ui).background):SetActive(false)
      ;
      ((self.ui).movie):SetActive(false)
      sectorCtrl:ShowUnlockSectorEffect(unlockSectorId, function()
        -- function num : 0_1_0_0_0 , upvalues : sectorId, _ENV, sectorCtrl
        if sectorId == 5 then
          local sectorStage = ((ConfigData.sector_stage).sectorIdList)[6]
          ...
          if (PlayerDataCenter.sectorStage):IsStageComplete(stageId) then
            sectorCtrl:OnSectorItemClicked(6)
          else
