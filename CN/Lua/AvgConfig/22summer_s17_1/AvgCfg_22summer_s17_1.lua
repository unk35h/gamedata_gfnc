-- params : ...
-- function num : 0 , upvalues : _ENV
local AvgCfg_22summer_s17_1 = {
[1] = {bgColor = 2, content = 10, contentType = 2, 
images = {
{imgId = 1, imgType = 2, alpha = 0, imgPath = "cpt00/cpt00_e_cg015", fullScreen = true}
, 
{imgId = 2, imgType = 2, alpha = 0, imgPath = "cpt00/cpt00_e_cg015", fullScreen = true}
, 
{imgId = 3, imgType = 2, alpha = 0, order = 5, imgPath = "cpt05/cpt05_e_bg005", fullScreen = true}
, 
{imgId = 102, imgType = 3, alpha = 0, imgPath = "anna_avg"}
, 
{imgId = 16, imgType = 3, alpha = 0, imgPath = "faith2_avg"}
}
, 
imgTween = {
{imgId = 1, delay = 0, duration = 1, alpha = 1}
}
, 
audio = {
bgm = {stop = false, cue = "Mus_Story_DeusExMachina", sheet = "Mus_Story_DeusExMachina", fadeIn = 3, fadeOut = 1}
, 
sfx = {cue = "AVG_Slime_Movement", sheet = "AVG"}
}
}
, 
[2] = {content = 20, contentType = 2}
, 
[3] = {content = 30, contentType = 3, speakerHeroId = 1002, 
imgTween = {
{imgId = 1, delay = 0, duration = 0.6, alpha = 0}
, 
{imgId = 2, delay = 0.6, duration = 0.6, alpha = 1}
, 
{imgId = 102, delay = 0, duration = 0, posId = 3, alpha = 0, isDark = false}
, 
{imgId = 102, delay = 1.2, duration = 0.2, posId = 3, alpha = 1, isDark = false}
}
, 
heroFace = {
{imgId = 102, faceId = 14}
}
}
, 
[4] = {content = 40, contentType = 3, speakerHeroId = 1002, 
heroFace = {
{imgId = 102, faceId = 6}
}
}
, 
[5] = {content = 50, contentType = 4, speakerName = 11, 
imgTween = {
{imgId = 102, delay = 0, duration = 0.2, posId = 3, alpha = 1, isDark = true}
}
}
, 
[6] = {content = 60, contentType = 4, speakerName = 11, contentShake = true}
, 
[7] = {content = 70, contentType = 3, speakerHeroId = 1002, 
imgTween = {
{imgId = 102, delay = 0, duration = 0.2, posId = 3, alpha = 1, isDark = false}
}
, 
heroFace = {
{imgId = 102, faceId = 14}
}
}
, 
[8] = {content = 80, contentType = 3, speakerHeroId = 1002, 
heroFace = {
{imgId = 102, faceId = 6}
}
}
, 
[9] = {content = 90, contentType = 3, speakerHeroId = 16, 
imgTween = {
{imgId = 102, delay = 0, duration = 0.2, posId = 3, alpha = 0, isDark = false}
, 
{imgId = 16, delay = 0, duration = 0, posId = 3, alpha = 0, isDark = false}
, 
{imgId = 16, delay = 0, duration = 0.2, posId = 3, alpha = 1, isDark = false}
}
}
, 
[10] = {content = 100, contentType = 3, speakerHeroId = 16, contentShake = true, 
heroFace = {
{imgId = 16, faceId = 4}
}
}
, 
[11] = {content = 110, contentType = 3, speakerHeroId = 1005, 
images = {
{imgId = 105, imgType = 3, alpha = 0, imgPath = "croque_avg", comm = true}
}
, 
imgTween = {
{imgId = 16, delay = 0, duration = 0.2, posId = 3, alpha = 0, isDark = false}
, 
{imgId = 105, delay = 0, duration = 0, posId = 3, 
rot = {0, 180, 0}
, alpha = 0, isDark = false}
, 
{imgId = 105, delay = 0, duration = 0.2, posId = 3, alpha = 1, isDark = false}
}
, 
heroFace = {
{imgId = 105, faceId = 6}
}
}
, 
[12] = {content = 120, contentType = 3, speakerHeroId = 1005}
, 
[13] = {content = 130, contentType = 3, speakerHeroId = 1005}
, 
[14] = {content = 140, contentType = 3, speakerHeroId = 1002, speakerHeroPosId = 3, contentShake = true, 
imgTween = {
{imgId = 105, delay = 0, duration = 0.6, posId = 2, alpha = 1, isDark = true}
, 
{imgId = 102, delay = 0, duration = 0, posId = 4, alpha = 0, isDark = false}
, 
{imgId = 102, delay = 0, duration = 0.2, posId = 4, alpha = 1, isDark = false}
}
}
, 
[15] = {content = 150, contentType = 3, speakerHeroId = 1005, speakerHeroPosId = 1, contentShake = true, 
imgTween = {
{imgId = 102, delay = 0, duration = 0.2, posId = 4, alpha = 1, isDark = true}
, 
{imgId = 105, delay = 0, duration = 0.2, posId = 2, alpha = 1, isDark = false}
}
}
, 
[16] = {autoContinue = true, 
images = {
{imgId = 105, imgType = 3, alpha = 0, imgPath = "croque_avg", delete = true}
}
, 
imgTween = {
{imgId = 2, delay = 0, duration = 1, alpha = 0}
, 
{imgId = 102, delay = 0, duration = 0.2, posId = 4, alpha = 0, isDark = true}
}
}
}
return AvgCfg_22summer_s17_1

