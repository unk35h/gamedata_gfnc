-- params : ...
-- function num : 0 , upvalues : _ENV
local AvgCfg_22hallo_e_sol = {
[1] = {content = 10, contentType = 3, speakerHeroId = 1003, speakerHeroPosId = 2, 
images = {
{imgId = 103, imgType = 3, alpha = 0, imgPath = "sol_avg"}
, 
{imgId = 105, imgType = 3, alpha = 0, imgPath = "croque_avg", 
rot = {0, 180, 0}
}
}
, 
imgTween = {
{imgId = 103, delay = 0, duration = 0, posId = 3, alpha = 0, isDark = false}
, 
{imgId = 103, delay = 0, duration = 0.2, posId = 3, alpha = 1, isDark = false}
, 
{imgId = 103, delay = 0.6, duration = 0.2, posId = 3, alpha = 1, shake = true, isDark = false}
, 
{imgId = 103, delay = 0.8, duration = 0.2, posId = 3, alpha = 1, shake = true, isDark = false}
}
, 
heroFace = {
{imgId = 103, faceId = 8}
}
}
, 
[2] = {content = 20, contentType = 4, speakerName = 11, 
imgTween = {
{imgId = 103, delay = 0, duration = 0.2, posId = 3, alpha = 1, isDark = true}
}
}
, 
[3] = {content = 30, contentType = 3, speakerHeroId = 1005, speakerHeroPosId = 2, 
imgTween = {
{imgId = 103, delay = 0, duration = 0.2, posId = 3, alpha = 0, isDark = true}
, 
{imgId = 105, delay = 0, duration = 0, posId = 3, alpha = 0, isDark = false}
, 
{imgId = 105, delay = 0, duration = 0.6, posId = 3, alpha = 1, isDark = false}
}
, 
heroFace = {
{imgId = 105, faceId = 4}
}
}
, 
[4] = {content = 40, contentType = 4, speakerName = 11, 
imgTween = {
{imgId = 105, delay = 0, duration = 0.2, posId = 3, alpha = 1, isDark = true}
}
}
, 
[5] = {content = 50, contentType = 3, speakerHeroId = 1005, speakerHeroPosId = 2, 
imgTween = {
{imgId = 105, delay = 0, duration = 0.2, posId = 3, alpha = 1, isDark = false}
}
, 
heroFace = {
{imgId = 105, faceId = 0}
}
}
, 
[6] = {content = 60, contentType = 3, speakerHeroId = 1005, speakerHeroPosId = 2, 
heroFace = {
{imgId = 105, faceId = 4}
}
}
, 
[7] = {content = 70, contentType = 3, speakerHeroId = 1005, speakerHeroPosId = 2}
, 
[8] = {content = 80, contentType = 2, 
imgTween = {
{imgId = 105, delay = 0, duration = 0.2, posId = 3, alpha = 0, isDark = false}
}
}
, 
[9] = {content = 90, contentType = 4, speakerName = 12, contentShake = true, 
images = {
{imgId = 103, imgType = 3, alpha = 0, imgPath = "sol_avg", delete = true}
, 
{imgId = 105, imgType = 3, alpha = 0, imgPath = "croque_avg", 
rot = {0, 180, 0}
, delete = true}
, 
{imgId = 103, imgType = 3, alpha = 0, imgPath = "sol_avg", ripple = true}
, 
{imgId = 105, imgType = 3, alpha = 0, imgPath = "croque_avg", 
rot = {0, 180, 0}
, ripple = true}
}
, 
imgTween = {
{imgId = 103, delay = 0, duration = 0, posId = 2, alpha = 0, isDark = false}
, 
{imgId = 103, delay = 0, duration = 0.2, posId = 2, alpha = 1, isDark = false}
, 
{imgId = 103, delay = 0.6, duration = 0.2, posId = 2, alpha = 1, shake = true, isDark = false}
, 
{imgId = 103, delay = 0.8, duration = 0.2, posId = 2, alpha = 1, shake = true, isDark = false}
}
, 
heroFace = {
{imgId = 103, faceId = 15}
}
}
, 
[10] = {content = 100, contentType = 4, speakerName = 13, 
imgTween = {
{imgId = 103, delay = 0, duration = 0.2, posId = 2, alpha = 1, isDark = true}
, 
{imgId = 105, delay = 0, duration = 0, posId = 4, alpha = 0, isDark = false}
, 
{imgId = 105, delay = 0, duration = 0.2, posId = 4, alpha = 1, isDark = false}
}
, 
heroFace = {
{imgId = 105, faceId = 2}
}
}
, 
[11] = {content = 110, contentType = 4, speakerName = 12, contentShake = true, 
imgTween = {
{imgId = 103, delay = 0, duration = 0.2, posId = 2, alpha = 1, isDark = false, shake = true, shakeIntensity = 4}
, 
{imgId = 105, delay = 0, duration = 0.2, posId = 4, alpha = 1, isDark = true}
}
, 
heroFace = {
{imgId = 103, faceId = 8}
}
}
, 
[12] = {content = 120, contentType = 4, speakerName = 13, contentShake = true, 
imgTween = {
{imgId = 103, delay = 0, duration = 0.6, posId = 1, alpha = 0, isDark = false}
, 
{imgId = 105, delay = 0.6, duration = 0.2, posId = 4, alpha = 1, isDark = false, shake = true}
}
, 
heroFace = {
{imgId = 105, faceId = 6}
}
}
, 
[13] = {content = 130, contentType = 4, speakerName = 12, contentShake = true, 
imgTween = {
{imgId = 105, delay = 0, duration = 0.2, posId = 4, alpha = 1, isDark = true}
}
}
, 
[14] = {content = 140, contentType = 4, speakerName = 13, 
imgTween = {
{imgId = 105, delay = 0, duration = 0.2, posId = 4, alpha = 1, isDark = false, shake = true}
}
, 
heroFace = {
{imgId = 105, faceId = 5}
}
}
, 
[15] = {content = 150, contentType = 4, speakerName = 12, contentShake = true, 
imgTween = {
{imgId = 105, delay = 0, duration = 0.2, posId = 4, alpha = 1, isDark = true}
}
}
, 
[16] = {content = 160, contentType = 4, speakerName = 13, 
imgTween = {
{imgId = 105, delay = 0, duration = 0.2, posId = 4, alpha = 1, isDark = false}
}
, 
heroFace = {
{imgId = 105, faceId = 4}
}
, 
audio = {
sfx = {cue = "Atk_Snooper_Hit", sheet = "Mon_Snooper"}
}
}
, 
[17] = {content = 170, contentType = 2, 
imgTween = {
{imgId = 105, delay = 0, duration = 0.2, posId = 4, alpha = 0, isDark = true}
}
}
, 
[18] = {content = 180, contentType = 4, speakerName = 11, 
images = {
{imgId = 103, imgType = 3, alpha = 0, imgPath = "sol_avg"}
, 
{imgId = 105, imgType = 3, alpha = 0, imgPath = "croque_avg", 
rot = {0, 180, 0}
}
, 
{imgId = 103, imgType = 3, alpha = 0, imgPath = "sol_avg", ripple = true, delete = true}
, 
{imgId = 105, imgType = 3, alpha = 0, imgPath = "croque_avg", 
rot = {0, 180, 0}
, ripple = true, delete = true}
}
}
, 
[19] = {content = 190, contentType = 3, speakerHeroId = 1005, speakerHeroPosId = 2, 
imgTween = {
{imgId = 105, delay = 0, duration = 0, posId = 3, alpha = 0, isDark = false}
, 
{imgId = 105, delay = 0, duration = 0.2, posId = 3, alpha = 1, isDark = false}
}
, 
heroFace = {
{imgId = 105, faceId = 4}
}
}
, 
[20] = {content = 200, contentType = 3, speakerHeroId = 1005, 
heroFace = {
{imgId = 105, faceId = 0}
}
}
}
return AvgCfg_22hallo_e_sol

