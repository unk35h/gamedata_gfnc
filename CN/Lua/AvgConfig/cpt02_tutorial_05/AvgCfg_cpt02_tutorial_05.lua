-- params : ...
-- function num : 0 , upvalues : _ENV
local AvgCfg_cpt02_tutorial_05 = {
[1] = {
ppv = {
cg = {saturation = -50}
}
, 
images = {
{imgId = 101, imgType = 3, alpha = 0, imgPath = "simo_shadow_avg", 
rot = {0, 180, 0}
}
}
, content = 10, contentType = 4, speakerName = 11, 
imgTween = {
{imgId = 101, delay = 0, duration = 0, posId = 5, alpha = 0, isDark = false}
, 
{imgId = 101, delay = 0, duration = 0.6, posId = 4, alpha = 0.75, isDark = false}
}
}
, 
[2] = {content = 20, contentType = 4, speakerName = 11}
, 
[3] = {content = 30, contentType = 4, speakerName = 11}
, 
[4] = {content = 40, contentType = 4, speakerName = 11}
, 
[5] = {content = 50, contentType = 4, speakerName = 11}
, 
[6] = {content = 60, contentType = 4, speakerName = 11}
, 
[7] = {content = 70, contentType = 4, speakerName = 11}
, 
[8] = {content = 80, contentType = 4, speakerName = 11}
, 
[9] = {content = 90, contentType = 4, speakerName = 11, 
imgTween = {
{imgId = 101, delay = 0, duration = 0.6, posId = 4, alpha = 0, isDark = false}
}
}
}
return AvgCfg_cpt02_tutorial_05

