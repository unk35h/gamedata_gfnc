-- params : ...
-- function num : 0 , upvalues : _ENV
local AvgCfg_22hallo_e_clotho = {
[1] = {content = 10, contentType = 4, speakerName = 12, contentShake = true, 
images = {
{imgId = 167, imgType = 3, alpha = 0, imgPath = "clotho_avg"}
, 
{imgId = 143, imgType = 3, alpha = 0, imgPath = "hel_avg"}
}
, 
imgTween = {
{imgId = 143, delay = 0, duration = 0, posId = 3, alpha = 0, isDark = false}
, 
{imgId = 143, delay = 0, duration = 0.2, posId = 3, alpha = 1, isDark = false}
, 
{imgId = 143, delay = 0.2, duration = 1, posId = 3, alpha = 1, shake = true, isDark = false}
}
, 
heroFace = {
{imgId = 143, faceId = 3}
}
}
, 
[2] = {content = 20, contentType = 4, speakerName = 11, 
imgTween = {
{imgId = 143, delay = 0, duration = 0.2, posId = 3, alpha = 1, isDark = true}
}
}
, 
[3] = {content = 30, contentType = 3, speakerHeroId = 1067, speakerHeroPosId = 1, 
imgTween = {
{imgId = 167, delay = 0, duration = 0, posId = 2, alpha = 0, isDark = false}
, 
{imgId = 167, delay = 0, duration = 0.2, posId = 2, alpha = 1, isDark = false}
, 
{imgId = 143, delay = 0, duration = 0.6, posId = 4, alpha = 1, isDark = true}
}
, 
heroFace = {
{imgId = 167, faceId = 4}
}
}
, 
[4] = {content = 40, contentType = 4, speakerName = 11, 
imgTween = {
{imgId = 167, delay = 0, duration = 0.2, posId = 2, alpha = 1, isDark = true}
}
}
, 
[5] = {content = 50, contentType = 3, speakerHeroId = 143, speakerHeroPosId = 3, contentShake = true, 
imgTween = {
{imgId = 143, delay = 0, duration = 0.2, posId = 4, alpha = 1, isDark = false}
}
, 
heroFace = {
{imgId = 143, faceId = 0}
}
}
, 
[6] = {content = 60, contentType = 3, speakerHeroId = 143, speakerHeroPosId = 3}
, 
[7] = {content = 70, contentType = 4, speakerName = 11, 
imgTween = {
{imgId = 143, delay = 0, duration = 0.2, posId = 4, alpha = 1, isDark = true}
}
}
, 
[8] = {content = 80, contentType = 4, speakerName = 12, 
imgTween = {
{imgId = 143, delay = 0, duration = 0.2, posId = 4, alpha = 1, shake = true, isDark = false}
, 
{imgId = 143, delay = 0.6, duration = 0.2, posId = 4, alpha = 1, shake = true, isDark = false}
}
, 
heroFace = {
{imgId = 143, faceId = 1}
}
}
, 
[9] = {content = 90, contentType = 4, speakerName = 12, contentShake = true, 
imgTween = {
{imgId = 143, delay = 0, duration = 1, posId = 4, alpha = 1, shake = true, isDark = false}
}
, 
heroFace = {
{imgId = 143, faceId = 2}
}
}
, 
[10] = {content = 100, contentType = 3, speakerHeroId = 143, speakerHeroPosId = 3, 
heroFace = {
{imgId = 143, faceId = 0}
}
}
, 
[11] = {content = 110, contentType = 4, speakerName = 11, 
imgTween = {
{imgId = 143, delay = 0, duration = 0.2, posId = 4, alpha = 1, isDark = true}
}
}
, 
[12] = {content = 120, contentType = 3, speakerHeroId = 143, speakerHeroPosId = 3, 
imgTween = {
{imgId = 143, delay = 0, duration = 0.2, posId = 4, alpha = 1, isDark = false}
}
}
, 
[13] = {content = 130, contentType = 3, speakerHeroId = 1067, speakerHeroPosId = 1, 
imgTween = {
{imgId = 143, delay = 0, duration = 0.2, posId = 4, alpha = 1, isDark = true}
, 
{imgId = 167, delay = 0, duration = 0.2, posId = 2, alpha = 1, isDark = false}
}
}
, 
[14] = {content = 140, contentType = 4, speakerName = 13, contentShake = true, 
imgTween = {
{imgId = 143, delay = 0, duration = 0.2, posId = 4, alpha = 1, isDark = false}
, 
{imgId = 143, delay = 0.2, duration = 1, posId = 4, alpha = 1, shake = true, isDark = false}
, 
{imgId = 167, delay = 0, duration = 0.2, posId = 2, alpha = 1, isDark = true}
}
, 
heroFace = {
{imgId = 143, faceId = 2}
}
}
, 
[15] = {content = 150, contentType = 3, speakerHeroId = 143, speakerHeroPosId = 3, contentShake = true, 
imgTween = {
{imgId = 143, delay = 0, duration = 0.2, posId = 4, alpha = 1, shake = true, isDark = false}
}
, 
heroFace = {
{imgId = 143, faceId = 3}
}
}
, 
[16] = {content = 160, contentType = 2, 
imgTween = {
{imgId = 143, delay = 0, duration = 0.2, posId = 4, alpha = 0, isDark = false}
, 
{imgId = 167, delay = 0, duration = 0.2, posId = 2, alpha = 0, isDark = false}
}
}
, 
[17] = {content = 170, contentType = 4, speakerName = 11}
, 
[18] = {content = 180, contentType = 3, speakerHeroId = 143, speakerHeroPosId = 2, contentShake = true, 
imgTween = {
{imgId = 143, delay = 0, duration = 0, posId = 3, alpha = 0, isDark = false}
, 
{imgId = 143, delay = 0, duration = 0.2, posId = 3, alpha = 1, shake = true, isDark = false}
}
, 
heroFace = {
{imgId = 143, faceId = 0}
}
}
}
return AvgCfg_22hallo_e_clotho

