-- params : ...
-- function num : 0 , upvalues : _ENV
local AvgCfg_cpt_hb_tutorial_04 = {
[1] = {content = 10, contentType = 3, speakerHeroId = 1053, speakerHeroPosId = 2, contentShake = true, 
images = {
{imgId = 153, imgType = 3, alpha = 0, imgPath = "sueyoi_avg"}
, 
{imgId = 101, imgType = 3, alpha = 0, imgPath = "persicaria_avg", ripple = true}
, 
{imgId = 103, imgType = 3, alpha = 0, imgPath = "sol_avg", ripple = true}
, 
{imgId = 156, imgType = 3, alpha = 0, imgPath = "lind_avg", ripple = true}
}
, 
imgTween = {
{imgId = 153, delay = 0, duration = 0, posId = 3, alpha = 0, isDark = false}
, 
{imgId = 153, delay = 0, duration = 0.6, posId = 3, alpha = 1, isDark = false}
}
, 
heroFace = {
{imgId = 153, faceId = 44}
}
}
, 
[2] = {content = 20, contentType = 2, contentShake = true, 
imgTween = {
{imgId = 153, delay = 0, duration = 0.2, posId = 3, alpha = 0, isDark = false}
}
}
, 
[3] = {content = 30, contentType = 3, speakerHeroId = 1053, speakerHeroPosId = 2, 
imgTween = {
{imgId = 153, delay = 0, duration = 0, posId = 3, alpha = 0, isDark = false}
, 
{imgId = 153, delay = 0, duration = 0.2, posId = 3, alpha = 1, shake = true, isDark = false}
}
}
, 
[4] = {content = 40, contentType = 2, 
imgTween = {
{imgId = 153, delay = 0, duration = 0.2, posId = 3, alpha = 0, isDark = false}
}
}
, 
[5] = {content = 50, contentType = 3, speakerHeroId = 1001, speakerHeroPosId = 1, 
imgTween = {
{imgId = 101, delay = 0, duration = 0, posId = 2, alpha = 0, isDark = false}
, 
{imgId = 101, delay = 0, duration = 0.2, posId = 2, alpha = 1, isDark = false}
, 
{imgId = 103, delay = 0, duration = 0, posId = 4, alpha = 0, isDark = true}
, 
{imgId = 103, delay = 0, duration = 0.2, posId = 4, alpha = 1, isDark = true}
}
, 
heroFace = {
{imgId = 101, faceId = 10}
, 
{imgId = 103, faceId = 11}
}
}
, 
[6] = {content = 60, contentType = 3, speakerHeroId = 1053, speakerHeroPosId = 2, 
imgTween = {
{imgId = 103, delay = 0, duration = 0.6, posId = 4, alpha = 0, isDark = true}
, 
{imgId = 101, delay = 0, duration = 0.6, posId = 2, alpha = 0, isDark = false}
, 
{imgId = 153, delay = 0, duration = 0, posId = 3, alpha = 0, isDark = false}
, 
{imgId = 153, delay = 0.6, duration = 0.2, posId = 3, alpha = 1, isDark = false}
}
, 
heroFace = {
{imgId = 153, faceId = 33}
}
}
, 
[7] = {content = 70, contentType = 3, speakerHeroId = 1001, speakerHeroPosId = 1, 
imgTween = {
{imgId = 153, delay = 0, duration = 0.2, posId = 3, alpha = 0, isDark = false}
, 
{imgId = 101, delay = 0, duration = 0, posId = 2, alpha = 0, isDark = false}
, 
{imgId = 101, delay = 0, duration = 0.2, posId = 2, alpha = 1, isDark = false}
, 
{imgId = 103, delay = 0, duration = 0, posId = 4, alpha = 0, isDark = true}
, 
{imgId = 103, delay = 0, duration = 0.2, posId = 4, alpha = 1, isDark = true}
}
}
, 
[8] = {content = 80, contentType = 3, speakerHeroId = 1003, speakerHeroPosId = 3, 
imgTween = {
{imgId = 103, delay = 0, duration = 0.2, posId = 4, alpha = 1, isDark = false}
, 
{imgId = 101, delay = 0, duration = 0.2, posId = 2, alpha = 1, isDark = true}
}
, 
heroFace = {
{imgId = 103, faceId = 11}
}
}
, 
[9] = {content = 90, contentType = 3, speakerHeroId = 1001, speakerHeroPosId = 1, 
imgTween = {
{imgId = 103, delay = 0, duration = 0.2, posId = 4, alpha = 1, isDark = true}
, 
{imgId = 101, delay = 0, duration = 0.2, posId = 2, alpha = 1, isDark = false}
}
, 
heroFace = {
{imgId = 101, faceId = 5}
}
}
, 
[10] = {content = 100, contentType = 3, speakerHeroId = 1001, speakerHeroPosId = 1}
, 
[11] = {content = 110, contentType = 3, speakerHeroId = 1003, speakerHeroPosId = 2, contentShake = true, 
imgTween = {
{imgId = 101, delay = 0, duration = 0.6, posId = 2, alpha = 0, isDark = false}
, 
{imgId = 103, delay = 0, duration = 0.6, posId = 3, alpha = 1, isDark = false}
}
, 
heroFace = {
{imgId = 103, faceId = 9}
}
}
, 
[12] = {content = 120, contentType = 2, contentShake = true, 
imgTween = {
{imgId = 103, delay = 0, duration = 0.2, posId = 3, alpha = 0, isDark = true}
}
}
, 
[13] = {autoContinue = true}
, 
[14] = {content = 140, contentType = 3, speakerHeroId = 1001, 
imgTween = {
{imgId = 101, delay = 0, duration = 0, posId = 3, alpha = 0, isDark = false}
, 
{imgId = 101, delay = 0, duration = 0.6, posId = 3, alpha = 1, isDark = false}
}
}
, 
[15] = {autoContinue = true, isEnd = true, 
imgTween = {
{imgId = 101, delay = 0, duration = 0.6, posId = 3, alpha = 0, isDark = false}
}
}
}
return AvgCfg_cpt_hb_tutorial_04

