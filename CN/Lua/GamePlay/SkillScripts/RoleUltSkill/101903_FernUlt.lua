...
, buffId_stun = 3006, buffId_tip = 101901, selectId = 9, select_Range = 10, effect_start = 101901, effect_hit = 101902, effect_MoveSpeed = 7, buffFeature_ignoreCtrl = 15, buffId_Dj = 100701, buffId_Back = 3007, audioIdStart = 101901, audioIdMovie = 101902, audioIdEnd = 101903}
  ...
  for i = 0, EnemyList.Count - 1 do
    ...
    if role.hp > 0 then
      ...
      if role ~= nil and role.belongNum ~= eBattleRoleBelong.neutral and IfRoleCotainsIgnoreCtrlBuff == false then
        if role:GetBuffTier((self.config).buffId_Dj) > 0 then
