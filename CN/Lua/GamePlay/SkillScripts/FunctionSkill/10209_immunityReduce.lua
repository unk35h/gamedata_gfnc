...
bs_10209.OnSetHurt = function(self, context)
  ...
  if context.sender == self.caster and not context.isMiss and LuaSkillCtrl:CallRange(1, 1000) <= (self.arglist)[1] and not context.isTriggerSet and context.hurt_type == 1 and (self.arglist)[3] > 0 and (self.arglist)[4] > 0 then
    local tier = 1
    self:PlayChipEffect()
