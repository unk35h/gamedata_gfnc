...
bs_200102.CheckAndGetEmptyRole = function(self, roleList)
  ...
  for i = 0, roleList.Count - 1 do
    ...
    if targetRole.intensity > 0 and targetRole ~= nil and targetRole.lastPreSetGrid == nil and ((targetRole.recordTable).isPhaseSelect == nil or not (targetRole.recordTable).isPhaseSelect) then
      if LuaSkillCtrl:IsRoleAdjacent(self.caster, targetRole) then
        dontNeedMove = true
...
bs_200102.CheckAbandonRoleNextRoutes = function(self, targetRoles, emptyGrid)
  -- function num : 0_6
  for i = 0, targetRoles.Count - 1 do
    local targetRole = (targetRoles[i]).targetRole
    local nextRoutes = targetRole.nextCoordRoute
    if nextRoutes ~= nil and nextRoutes.Count > 0 then
